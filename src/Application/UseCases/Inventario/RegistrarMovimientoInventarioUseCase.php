...
class RegistrarMovimientoInventarioUseCase
{
    ...
    public function ejecutar(int $id_inventario, int $cantidad, string $tipo_movimiento, ?string $motivo = null): void
    {
        if ($cantidad <= 0) {
            throw new Exception("La cantidad debe ser un número positivo.");
        }

        $this->repository->registrarMovimiento($id_inventario, $cantidad, $tipo_movimiento, $motivo);
    }
}